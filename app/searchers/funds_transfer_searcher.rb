class FundsTransferSearcher < Searcher
  attr_searchable :req_no, :debit_account_no, :from_transfer_amount, :to_transfer_amount, :from_req_timestamp, :to_req_timestamp, 
                  :from_rep_timestamp, :to_rep_timestamp, :transfer_type, :bene_account_ifsc, :status_code, :customer_id, :op_name, :ws_saf
  ...
  as_enum :op_name, [:startTransfer, :transfer], map: :string, source: :op_name
  ...
  def find
    ...
    reln = reln.where("was_saf in (?,NULL)",ws_saf) if ws_saf.present? and ws_saf == 'N'

class FtApbsIncomingRecordSearcher
  ...
  def find
    ...
    reln = reln.where("ft_apbs_incoming_records.apbs_trans_code LIKE ? ", "#{apbs_trans_code}%"}) if apbs_trans_code.present?
    reln = reln.where("ft_apbs_incoming_records.dest_bank_iin LIKE ? ", "#{dest_bank_iin}%") if dest_bank_iin.present?
    reln = reln.where("ft_apbs_incoming_records.sponser_bank_iin LIKE ? ", "#{sponser_bank_iin}%") if sponser_bank_iin.present?
    reln = reln.where("ft_apbs_incoming_records.bene_acct_name LIKE ? ", "#{bene_acct_name}%") if bene_acct_name.present?
    reln
  end
end